...
<div class="flex h-full flex-col gap-4 p-6">
  <section class="flex flex-col gap-3">
    <h2 class="text-sm font-bold uppercase tracking-wider text-muted">Select SGBD files</h2>
    <p class="max-w-2xl text-sm text-muted">
      Pick an ECU folder to surface every <code class="text-muted">.prg</code> /
      <code class="text-muted">.grp</code> inside it (recurses one level deep
      via <code class="text-muted">webkitdirectory</code>), or open a single file
    ...
    <div class="flex gap-3">
      <button
        type="button"
        class="rounded border border-rule bg-surface px-4 py-2 text-sm hover:border-accent hover:bg-elevated"
  ...
  <section
    ...
    class:border-divider={!isDragging}
  ...
  >
    ...
    {#if app.pickedFiles.length === 0}
      <div class="flex flex-1 items-center justify-center text-sm text-faint">
        Drag a folder or a <code class="mx-1 text-muted">.prg</code> /
        <code class="mx-1 text-muted">.grp</code> file here.
      </div>
    {:else}
      <div class="flex flex-1 flex-col overflow-hidden">
        <div class="border-b border-divider px-4 py-2 text-xs text-faint">
        ...
        <ul class="flex-1 overflow-auto">
          {#each app.pickedFiles as file (file.relativePath)}
            <li>
              <button
                type="button"
                class="flex w-full items-center gap-3 border-b border-divider px-4 py-2 text-left text-sm hover:bg-surface"
                class:bg-surface={app.loadedFile?.relativePath === file.relativePath}
                onclick={() => pick(file)}
              >
                <span class="text-xs uppercase text-accent">{file.ext}</span>
                <span class="flex-1 truncate text-foreground">{file.relativePath}</span>
                <span class="text-xs text-faint">{(file.file.size / 1024).toFixed(1)} KB</span>
              </button>
            </li>
          {/each}
        </ul>
